...
Comandi:
    ...
    /help     → comandi
    /status   → statistiche
    /update   → indicizza solo file nuovi (veloce)
    /reindex  → cancella tutto e re-indicizza
"""
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Puoi anche inviarmi file per aggiungerli alla knowledge base.\n\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"📄 Invia un file ({supported}) per aggiungerlo.",
        parse_mode="Markdown",
    )
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"Cartella: `{settings.documents_dir}`\n"
...
async def cmd_update(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("🔄 Cerco file nuovi...")

    try:
        report = ingestion_service.ingest_directory(settings.documents_dir, force=False)

        if report["files_processed"] == 0 and report["files_skipped"] > 0:
            await update.message.reply_text(
                f"✅ Tutto aggiornato, nessun file nuovo.\n"
                f"File saltati: {report['files_skipped']}\n"
                f"Chunk totali: {vector_store.count()}"
            )
        else:
            await update.message.reply_text(
                f"✅ *Aggiornamento completato*\n\n"
                f"File nuovi: {report['files_processed']}\n"
                f"File saltati: {report['files_skipped']}\n"
        ...
        logger.error(f"Errore durante update: {e}")
...
async def cmd_reindex(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("🔄 Re-indicizzazione completa in corso...")

    try:
        vector_store.clear()
        report = ingestion_service.ingest_directory(settings.documents_dir, force=True)

        text = (
            f"✅ *Re-indicizzazione completata*\n\n"
            f"File processati: {report['files_processed']}\n"
            f"Errori: {report['files_failed']}\n"
            f"Chunk creati: {report['chunks_created']}\n"
            f"Tempo: {report['elapsed_seconds']}s"
        )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f"🔄 Indicizzazione...", parse_mode="Markdown")

        report = ingestion_service.ingest_file(str(file_path))

        if report["files_failed"] > 0:
            await update.message.reply_text(
                f"❌ Errore: {report['errors'][0]['error']}",
                parse_mode="Markdown",
            )
        else:
            await update.message.reply_text(
...
async def post_init(application: Application) -> None:
    commands = [
        ...
        BotCommand("status", "Statistiche"),
        BotCommand("update", "Indicizza file nuovi"),
        BotCommand("reindex", "Re-indicizza tutto"),
    ]
...
def run_bot() -> None:
    ...
    if ALLOWED_USERS:
        ...
        logger.warning("Bot aperto a tutti")
    ...
    logger.info(f"Vector store: {vector_store.count()} chunk")
